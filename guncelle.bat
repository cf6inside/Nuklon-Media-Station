...
python -m pip install --upgrade yt-dlp customtkinter Pillow pyinstaller
...
if not exist "ffprobe.exe" (echo HATA: ffprobe.exe dosyasi programin yaninda yok! & pause & exit)
...
echo.
echo === 3. ASAMA: Paketleme Basliyor ===
python -m PyInstaller --onefile --noconsole --icon=ikon.ico --add-binary "ffmpeg.exe;." --add-binary "ffprobe.exe;." --add-data "logo.jpg;." --add-data "ikon.ico;." --name="Nuklon_Medya_Merkezi" indirici.py
